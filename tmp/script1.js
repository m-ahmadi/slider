...
// var h = (function () {
	// var timeout;
	// return {
		// setInterval: function (func, time, handleObj, handleProp) {
			// timeout = setTimeout(function () {
				// if (timeout > 0) {
					// h.setInterval(func, time, handleObj, handleProp);
					// try {
						// func();
					// } catch (e) {
						// throw e.toString();
					// }
					// return handleObj.handleProp;
				// }
			// }, time);
			// handleObj.handleProp = timeout;
		// },
...
function autoplaySlides() {
	slidesPlayback = true;
	playBullets();
	//h.setInterval(playSlides, SHOWING_TIME, handles, handles.slideID);
	setInterval(playSlides, SHOWING_TIME);
}
function pauseSlides() {
	slidesPlayback = false;
	//h.clearInterval(handles.slideID);
	clearInterval(handles.slideID);
}
function playSlides() {
...
function playSlide(slide) {
	if (slide > TOTAL_SLIDES || slide < (TOTAL_SLIDES - (TOTAL_SLIDES - 1)) ) { return undefined; }
...
function autoplayDescriptions() {
	descsPlayback = true;
	playDescriptions();
	//h.setInterval(playDescriptions, SHOWING_TIME, handles, handles.descID);
	setInterval(playDescriptions, SHOWING_TIME);
}

function pauseDescriptions() {
	descsPlayback = false;
	//h.clearInterval(handles.descID);
	clearInterval(handles.descID);
}
...
function forClosure(descNum, slide, delay) {
	setTimeout(function() {
		animateDesc(descNum, slide['desc'+descNum]);
	}, delay);
}	
function animateDesc(descClass, o) {
